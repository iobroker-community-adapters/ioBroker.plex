...
class Library
{
	 ...
	 *
	 */
	clean(string, lowerCase, replacement)
	{
		...
		string = string.replaceAll(/[^._\-/ :!#$%&()+=@^{}|~\p{Ll}\p{Lu}\p{Nd}]+/gu, replacement ? replacement : '#')
		...
		{ 
			this._adapter.getStates(state + '.*',async (err, states) =>
			{
				try
				{
					...
					this._adapter.log.debug(JSON.stringify(newConstant))
 ...
 *
 */
	readData(key, data, prefix, properties, expandNestedData)
	{
		...
		if (node.notExist) {
			newConstant[nodeKey] = {
				...
				"role": ((nodeKey.split('.').slice(-1))[0].toLowerCase() == 'mediaurl') ? "media.url" : 
	...
	convertNode(node, data)
	{
		...
		switch(node.convert.func)
		{
			...
			case "create-link-only":
				...
				if (node.convert.func == 'create-link-only') return link
				this.set(
					{
