...
class PlexHttp {
     ...
     * @param {string} opts.hostname         Plex server hostname or IP
     * @param {number} [opts.port]           Plex server port (default 32400)
     * @param {boolean} [opts.https]         Use HTTPS (default false unless port=443)
     * @param {string} [opts.token]          X-Plex-Token used for authentication
     * @param {number} [opts.timeout]        Per-request timeout in ms (default 10000)
     * @param {object} [opts.requestOptions] Extra axios config (e.g. cert, key, ca, httpsAgent)
     * @param {object} [opts.options]        Plex client metadata headers (identifier, product, version, ...)
     */
    constructor(opts) {
     ...
     *
     * @param {string} path           Request path relative to the Plex server root (must start with `/`)
     * @param {object} [opts]         Optional request overrides
     * @param {string} [opts.method]  HTTP method (default GET)
     * @param {object} [opts.headers] Extra request headers to merge in
     * @param {any} [opts.data]       Request body (for POST/PUT)
    ...
    async query(path, opts) {
        ...
        if (response.status < 200 || response.status > 299) {
            throw Object.assign(
                new Error(`Plex Server responded with HTTP ${response.status} (${statusText(response.status)})`),
...
function normalizeError(err) {
    ...
    if (err.code && NETWORK_ERROR_CODES.includes(err.code)) {
        return Object.assign(new Error(`${err.code}: ${err.message || err.code}`), { code: err.code });
    }
    // Map axios-specific error codes to the closest equivalent native code
    if (err.code === 'ERR_CANCELED') {
        return Object.assign(new Error(`ETIMEDOUT: request canceled`), { code: 'ETIMEDOUT' });
    }
    if (err.code === 'ERR_NETWORK') {
        return Object.assign(new Error(`ECONNREFUSED: ${err.message || 'network error'}`), { code: 'ECONNREFUSED' });
    }
    return err;
}
