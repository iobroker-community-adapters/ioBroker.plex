...
class Player
{
    ...
    constructor(adapter, options = {}, library)
	{
		...
		this.config.controllable 	= !!this._library.getDeviceState(`${this.prefix}.Player.controllable`) 
		this.refreshDetails 		= !!this._library.getDeviceState(`${this.prefix}._Controls.timeline.refreshDetails`) 
		...
		this._adapter.setTimeout((self) => {self.setControls();self.startUpdater()},200,this)
    }
			...
			try {
				...
				if (r.length == 0) this.details.type = 'all'
					...
					Object.keys(this.details).forEach((key) => {
						...
						let val = node && node.type && this._library.convertToType(this.details[key], node.type)
						if (node) {
						...
						val = node && node.type && this._library.convertToType(this.details[key], node.type) 
						if (node) {
							this.details[key] = val
	...
	setControls() {
		...
		{
			if(!this.config.controllable) this._library.set({node: controls, role: 'channel', description: 'Playback & Navigation Controls'});
			...
			for (let key in _Actions[mode])
			{
				...
				if (this._library.getDeviceState(controls + '.' + mode + '.' + key) === null) {
					this._library.set({
						'node': controls + '.' + mode + '.' + key,
						'description': mode.slice(0,1).toUpperCase()+mode.slice(1)+' ' + this._library.ucFirst(button.description),
	...
	async action(actionVal) {
		if (_Actions[actionVal.mode] !== undefined && _Actions[actionVal.mode][actionVal.action] !== undefined && this.address && this.port)
		{
			...
			if (_Actions[actionVal.mode][actionVal.action]["saveToPlayer"] == undefined) {
				...
				try {
					...
					// confirm commands
					this._library.confirmNode({node: `${actionVal.id}.${actionVal.mode}.${actionVal.action}`}, actionVal.val)
				} catch(err) {
				...
				let a = _Actions[actionVal.mode][actionVal.action].key
				this[_Actions[actionVal.mode][actionVal.action].key] = newVal
				this.startUpdater()
				this._library.confirmNode({node: `${actionVal.id}.${actionVal.mode}.${actionVal.action}`}, actionVal.val)
			}
		}
