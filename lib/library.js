...
class Library
{
	...
	_setValue(state, value, options = {})
	{
		if (state !== undefined)
		{
			if (value !== undefined && (options.force || this._STATES[state] === undefined || this._STATES[state] === null || this._STATES[state].val != value))
			{
				...
				this._setValueAsync(state, value);
			}
			else
	...
	/**
	 * set Value after role check. Is a hack to avoid setState() channels/devices
	 *
	 * @param	void
	 * @return	void
	 *
	 */
	async _setValueAsync(state, value)
	{
		try {
			let o = await this._adapter.getObjectAsync('plex.' + this._adapter.instance +'.'+ state)
			if (o.type === 'state') {	
