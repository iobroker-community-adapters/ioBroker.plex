...
class Player {
	...
	async updateTimeline() {
		...
		try {
			...
			try {
				...
				if (r.length == 0) this.details.type = 'all'
					...
					if (this.details.music.state === 'played'  && this.lyric) this.lyric.updateTime(time)
		...
		try {
			if (saveValues.metadata != this.details.key) {
						...
						switch (complex.func) {
							case 'lyric': {
								...
								if (index > -1) {
									dp = keys[index]
									if (data[key] && data[dp]) {
										if (this.lyric) this.lyric.updateData(data[dp], dp.split('.').slice(0,-1).join('.'))
										else this.lyric = new Lyric(this._controller._adapter, this._controller._library, data[dp], this.prefix + '.' + dp.split('.').slice(-1).join('.'))
									}
...
class Lyric { 
	...
	updateTime(ms) {
		let result = this.lyric.filter((l) => l.startOffset <= ms && l.endOffset >= ms).join(' - ') || '';
		this._library.set({node: prefix+'.currentText', role: "text", type: "string", description: "Lyrics currently being played"}, result);
	...
	async updateData(key, prefix) {
		...
		if (this.key && this.prefix) {
			...
			let options = {
				...
				'method': 'GET',
				'url': `${/*this._library.AXIOS_OPTIONS._protocol*/'http:'}//${this._adapter.config.plexIp}:${this._adapter.config.plexPort}${key}?X-Plex-Token=${this._adapter.config.plexToken}`,
			...
			if (result) {
				...
				this._library.set({node: prefix+'.fullText', role: "json", type: "json", description: "Complete lyrics currently being played as an array"}, JSON.stringify(this.fullText));
				this._library.set({node: prefix+'.currentText', role: "text", type: "string", description: "Lyrics currently being played"}, '');
				this._adapter.log.debug(`Lyric: ${JSON.stringify(this.fullText)}`);
			}
			//} catch(error) {
			//	this._controller._adapter.log.debug(`Error(141) ${error.toJson()}`);
			//}
		}
