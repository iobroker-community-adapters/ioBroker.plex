...
class Controller {
     ...
     * @param   {{JSON}}    [options={}] Json with config
	 */
...
class Player {
    ...
    setNotificationData(data) {
        if (data && typeof data === 'object') {
            ...
            for (const d in data.Player) if (data.Player[d] === 'undefined') delete data.Player[d];
            ...
            for (const key in data) this._controller._library.readData(`${this.prefix}.${key}`, data[key], this.prefix);
        ...
        try {
            ...
            try {
                ...
                for (const d in r) {
                    ...
                    for (const a of ['address', 'containerKey', 'guid', 'machineIdentifier', 'audioStreamID', 'videoStreamID']) {
                        delete r[d].$[a];
                    }
    ...
    setControls() {
        ...
        this.config.protocolCapabilities && this.config.protocolCapabilities.split(',').forEach(mode => // e.g. "timeline,playback,navigation,mirror,playqueues"
        {
            ...
            for (const key in this._controller._actions[mode]) {
                const newVal = this._controller._actions[mode][key].default !== undefined ? this._controller._actions[mode][key].default : false;
    ...
    async action(actionVal) {
        if (this._controller._actions[actionVal.mode] !== undefined && this._controller._actions[actionVal.mode][actionVal.action] !== undefined && this.address && this.port) {
            ...
            this._controller._adapter.log.warn(`Error triggering ${actionVal.mode} action -${actionVal.action}- on player ${this.getReadableID()} with ip:${this.address} and port:${this.port}.! See debug log for details! Action not supported!`);
        }
    }
        ...
        try {
            ...
            for (let s of sessions.Metadata) {
                if (s.Player.machineIdentifier === this.config.uuid) {
                    ...
                    if (s.Player && s.Player.state) delete s.Player.state;
                    this.media = s.Media;
                    s.media = undefined;
                    //this.metadata = s
...
class Lyric {
    ...
    updateTime(ms) {
        ...
        function updater(self) {
            ...
            for (const r in result) result[r] = result[r].text;
    ...
    async updateData(key, prefix) {
        ...
        if (this.key && this.prefix) {
            ...
            try {
                ...
                if (result) {
                    const templyric = result.data.MediaContainer
