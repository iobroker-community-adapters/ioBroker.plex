...
class Library
{
    ...
    constructor(adapter, options, plex)
    {
        ...
        for (const a in this._nodes) if (a != a.toLowerCase()) this._adapter.log.warn(`${a} - ${a.toLowerCase()}`);
    ...
    {
        const result = this._nodes[this.clean(node, lowerCase)] ||
    ...
    replaceDescription(description, a,b) {
        ...
        else for (const obj in description) result[obj] = description[obj].replace(a, b);
        return result;
    }
    ...
    appendToDescription(description, app) {
        ...
        else for (const obj in description) result[obj] = description[obj]+app;
        return result;
    }
    /**
    ...
    getDeviceStateJson(state, property = 'val')
    {
        ...
        for (const id in this._STATES) {
            if (id.startsWith(`${state}.`)) {
                const val = this._STATES[id] !== undefined && this._STATES[id] ? (this._STATES[id][property] || false) : null;
    ...
    del(state, nested, callback)
    {
        ...
        this._createNode({ 'node': state, 'description': 'DELETED' }, () =>
        {
            ...
            that._adapter.getStates(nested ? state + '.*' : state, (err, objects) =>
            {
                ...
                {
                    that._adapter.delObject(object, () =>
                    {
