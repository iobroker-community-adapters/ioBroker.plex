...
const parser = new XMLParser({
    ...
    attributesGroupName: '$', // group attributes like xml2js ($)
    attributeNamePrefix: '', // no @_ prefix
...
class Player {
     ...
     * @param {string} actionVal.id - The state prefix used to confirm the action node.
    ...
    async updateData(key, prefix) {
        ...
        if (this.key && this.prefix) {
            ...
            try {
                ...
                this._adapter.log.debug(`Error(141) ${error && error.message ? error.message : String(error)}`);
            }
        }
    }
}
