...
class Controller
{
	...
	static garbageExcluded = ["Player.localAddress", "Player.port", "Player.protocolCapabilities", "Player.controllable", "Player.uuid", "Player.title", "Player.details.key", "Player.details.time"]
				...
				// after x timeouts/errors cancel it
					if (self.updateTrys++ > 2) {
						...
						if (error.message == 'timeout') {
							this._controller._adapter.log.debug(`Stop getting client details ${this.prefix} - ${this.config.protocolCapabilities} - ${this.address} - ${this.port}`);		
		...
		if (this._controller._actions[actionVal.mode] !== undefined && this._controller._actions[actionVal.mode][actionVal.action] !== undefined && this.address && this.port)
		{
			...
			if (this._controller._actions[actionVal.mode][actionVal.action]["convert"] !== undefined) {
				switch(this._controller._actions[actionVal.mode][actionVal.action]["convert"]) {
					...
					break;
					case "playKey":
						let key = newVal.split('#')[0];
						let offset = newVal.split('#').length == 2 ? newVal.split('#')[1] : 0;
						// hardcoded 
						
						attribute = `key=${key}&address=${this._controller._adapter.config.plexIp}`+
										`&port=${this._controller._adapter.config.plexPort}&machineIdentifier=${this._controller.getServerId()}&` +
										`offset=${offset}`
