 ...
 */
let playing = [], streams = 0, playingDevice = [];
const players = [];
                ...
                for (const state in states) {
                    for (const end of [
                        ...
                        if (state.endsWith(end.text) && state.indexOf('Metadata'+end.text) == -1) {
                            const prefix = state.replace(`${adapter.name}.${adapter.instance}.`, '').replace(end.text, '');
...
function setEvent(data, source, prefix) {
    ...
    if (prefix == '_playing') {
        ...
        const groupBy = data.Player.title && data.Player.uuid !== undefined ? library.clean(data.Player.title, true) + '-' + data.Player.uuid : 'unknown';
    ...
    if (playerIp && playerPort && playerIdentifier && playerTitle) {
        ...
        _axios(options).then(res => {
            xml.parseString(res.data, function (err, result) {
                ...
                for (const d in result.MediaContainer.Timeline) {
                    const data = result.MediaContainer.Timeline[d].$;
                    Object.keys(data).forEach((key) => {
                        const prefix = '_playing.' + library.clean(playerTitle, true) + '-' + playerIdentifier;
...
async function covertI18n(name, json) {
    ...
    const translations = {};
    for (const n in language) translations[language[n]] = await readJsonFromFile(name, language[n]);
    for (const key in json) {
        json[key].description = {};
        for (const n in language) json[key].description[language[n]] = translations[language[n]][key];
    }
    writeJsonToFile(name, json, true);
}

async function writeJsonToFile(name, json, isFile = false) {//.dataTest/admin
    const file = isFile ? './.dataTest/' + name + '.json' : './.dataTest/' + name + '/admin/i18n/en/translations.json';
